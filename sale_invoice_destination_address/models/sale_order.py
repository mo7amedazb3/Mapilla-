...
class SaleOrder(models.Model):
    ...
    destination_address = fields.Char(
        ...
        copy=True,
        help='المدينة أو العنوان الذي ستتجه إليه منتجات هذا الطلب.',
    )
    destination_address_id = fields.Many2one(
        ...
        check_company=True,
        ondelete='restrict',
        help='اختر المدينة أو وجهة تسليم منتجات هذا الطلب.',
    )
    ...
    def _onchange_destination_address_id(self):
        for order in self:
            order.destination_address = order.destination_address_id.name or False

    def _prepare_invoice(self):
        self.ensure_one()
        invoice_vals = super()._prepare_invoice()
        invoice_vals['destination_address'] = self.destination_address or ''
        invoice_vals['destination_address_id'] = self.destination_address_id.id or False
        return invoice_vals
